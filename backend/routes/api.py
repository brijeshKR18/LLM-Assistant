...
# Initialize LLM with offline configuration
try:
    llm = OllamaLLM(
        model=LLM_MODEL,
    ...
    logger.info({"message": f"Initialized global offline LLM with model: {LLM_MODEL}"})
...
class QueryInput(BaseModel):
    ...
    filename: str = None  # <-- Added for temp file support
...
async def query_llm(input: QueryInput):
    ...
    try:
        # Initialize LLM with strict offline configuration
        llm_instance = OllamaLLM(
            model=model,
            base_url="http://localhost:11434",  # Force local Ollama instance
            # Additional parameters to ensure offline operation
            temperature=0.7,
            top_k=50,
            top_p=0.9,
        ...
        logger.info({"message": f"Initialized offline LLM with model: {model}"})
    except Exception as e:
    ...
    try:
        ...
        # Create QA chain with strict context-only prompt
        from langchain.prompts import PromptTemplate
        
        # Custom prompt template that enforces context-only responses
        prompt_template = """You are an AI assistant that ONLY uses the provided context to answer questions. 
...
Answer based ONLY on the provided context:"""
    ...
    # Execute query with offline enforcement
    try:
        ...
        # Enforce offline query validation
...
async def _process_uploaded_file(filename: str):
    """Process uploaded file and return parsed document."""
    if not filename:
        return None
